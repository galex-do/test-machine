...
func (r *ProjectRepository) GetAll() ([]models.Project, error) {
        ...
        for rows.Next() {
                ...
                err := rows.Scan(&p.ID, &p.Name, &p.Description, &p.CreatedAt, &p.UpdatedAt, &p.TestSuitesCount)
                if err != nil {
                        return nil, err
                }
...
func (r *ProjectRepository) GetByID(id int) (*models.Project, error) {
        ...
        err := r.db.QueryRow(`
                ...
                WHERE p.id = $1
                GROUP BY p.id, p.name, p.description, p.created_at, p.updated_at
        `, id).Scan(&project.ID, &project.Name, &project.Description, &project.CreatedAt, &project.UpdatedAt, &project.TestSuitesCount)
